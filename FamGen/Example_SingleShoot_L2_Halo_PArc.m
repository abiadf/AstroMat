...
% % X0 = [.8234 0 .0224 0 .1343 0]; % [nondimensional] Northern L1 Halo; 
% T0 = 2.7838; % [nondimensional]
% % T0 = 2.7464; % [nondimensional]
...
% IC for Northern L2 Halo (Begin Planar)
X0 = [1.18089 0 -0.002709 0 -0.155899 0]; % [nondimensional] Northern L2 Halo;
T0 = 3.41547; % [nondimensional] 
T0_half = T0/2; % [nondimensional] 
...
hold on
grid on
plot(r_peri_store,stable_ind_mag(:,1),'.r','MarkerSize',10);
...
ylabel('|\nu_1|')
title('L_2 Halo - Stability Index, \nu, as a Function of JC')
hold off
...
hold on
grid on
plot(r_peri_store,stable_ind_mag(:,2),'.r','MarkerSize',10);
plot(r_peri_store,stable_ubnd,':k','LineWidth',1);
plot(r_peri_store,stable_lbnd,':k','LineWidth',1);
...
hold on
grid on
plot(r_peri_store,stable_ind_mag(:,3),'.r','MarkerSize',10);
plot(r_peri_store,stable_ubnd,':k','LineWidth',1);
plot(r_peri_store,stable_lbnd,':k','LineWidth',1);
...
hold on
grid on
plot(r_peri_store,Jac,'.r','MarkerSize',10);
xlabel('Perilune Radius [km]')
ylabel('Jacobi Constant [nd]')
title('L_2 Halo - Jacobi Constant as a Function of Perilune Radius')
hold off
...
hold on
grid on
plot(tf_store.*t_ch./86400,Jac,'.r','MarkerSize',10);
xlabel('Orbital Period [days]')
ylabel('Jacobi Constant [nd]')
title('L_2 Halo - Jacobi Constant as a Function of Orbital Period')
hold off
...
hold on
grid on
plot(Jac,tf_store.*t_ch./86400,'.r','MarkerSize',10);
xlabel('Jacobi Constant [nd]')
ylabel('Orbital Period [days]')
title('L_2 Halo - Orbital Period as a Function of Jacobi Constant')
hold off
...
[nrho_ind] = find( 1832 < r_peri_store & r_peri_store < 17390);
x0_L2_nrho = x0_store(nrho_ind,:);
tf_L2_nrho = tf_store(nrho_ind);
n_L2_nrho = length(nrho_ind);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Save Periodic Orbit Family as .mat File  %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% save('EM_L2_Halo_500_orbits','x0_store','tf_store')
% save('EM_L2_Halo_2000_orbits','x0_store','tf_store')
% save('EM_L2_NRHO_orbits','x0_L2_nrho','tf_L2_nrho','n_L2_nrho')
% save('EM_Interesting_orbits','X0','T0','x0_L2_nrho','tf_L2_nrho')
