function [Z0,t_var,t_bnd,xis_bnd,xis_plot,uis_plot] = L2LNearHeteroDiscrt(X,colt)
% function [Z0,t_var,t_bnd,xis_plot,uis_plot] = L2LNearHeteroDiscrt(X,colt)
%
...
%    xis_bnd   matrix of boundary node states, 7 x n_seg+1
...
n_seg_P2 = 20; % number of segments for Phase 2, unstable manifold
n_seg_P3 = 20; % number of segments for Phase 3, stable manifold
...
% Propagate to obtain boundary nodes of initial orbit extra revolution
[~,X_revi_bnd] = ode113(@(t,in)EOM_CR3BP_STM(t,in,mu),t_bnd_revi,[X_init phi0],options);

% Propagate to obtain boundary nodes of final orbit extra revolution
[~,X_revf_bnd] = ode113(@(t,in)EOM_CR3BP_STM(t,in,mu),t_bnd_revf,[X_fin phi0],options);
...
[~,X_tau1_bnd] = ode113(@(t,in)EOM_CR3BP_STM(t,in,mu),t_bnd_P1,[X_init phi0],options);

% Propagate to obtain variable nodes of stable orbit coast arc; Recall this is propagated in REVERSE time
[~,X_tau2_bnd] = ode113(@(t,in)EOM_CR3BP_STM(t,in,mu),t_bnd_P4,[X_fin phi0],options);
...
[V1_mono,D1_mono] = eig(mono_orb1); % note the transpose of mono_orb1 is the true monodromy matrix
[V2_mono,D2_mono] = eig(mono_orb2); % note the transpose of mono_orb2 is the true monodromy matrix
D1eigs_SortIn = diag(D1_mono)';
D2eigs_SortIn = diag(D2_mono)';

%%% NEED TO FIX SORTING %%%
% % Sort Eigenvalues
% [~,D1Ixs_SortOut] = SortEig(D1eigs_SortIn);
% [~,D2Ixs_SortOut] = SortEig(D2eigs_SortIn);
% 
% % Sort Eigenvectors
% V1_mono = V1_mono(:,D1Ixs_SortOut);
% V2_mono = V2_mono(:,D2Ixs_SortOut);
...
[~,X_alph1_bnd] = ode113(@(t,in)EOM_CR3BP_STM(t,in,mu),t_bnd_P2,[xm1 phi0],options);

% Propagate to obtain variable nodes of unstable manifold; Recall this is propagated in REVERSE time
[~,X_alph2_bnd] = ode113(@(t,in)EOM_CR3BP_STM(t,in,mu),t_bnd_P3,[xm2 phi0],options);
...
sis_var_P1 = repmat(S,[1 1 n_seg_P1]);
sis_var_P4 = repmat(S,[1 1 n_seg_P4]);

% Set manifold coast arc control and slack values
uis_var_P2 = repmat(U,[1 1 n_seg_P2]);
uis_var_P3 = repmat(U,[1 1 n_seg_P3]);
sis_var_P2 = repmat(S,[1 1 n_seg_P2]);
sis_var_P3 = repmat(S,[1 1 n_seg_P3]);
...
X_Ucst_bnd = [X_tau1_bnd(:,1:6) m0.*ones(n_seg_P1+1,1)];
X_Scst_bnd = [X_tau2_bnd(:,1:6) mf.*ones(n_seg_P4+1,1)];
...
X_Uman_bnd = [X_alph1_bnd(:,1:6) m0.*ones(n_seg_P2+1,1)];
X_Sman_bnd = [X_alph2_bnd(:,1:6) mf.*ones(n_seg_P3+1,1)];
...
xis_bnd_revi = reshape(X_Urev_bnd(:,1:n_state)',[n_state n_seg_revi+1]);
xis_bnd_revf = reshape(X_Srev_bnd(:,1:n_state)',[n_state n_seg_revf+1]);
xis_bnd_P1 = reshape(X_Ucst_bnd(:,1:n_state)',[n_state n_seg_P1+1]);
xis_bnd_P4 = reshape(X_Scst_bnd(:,1:n_state)',[n_state n_seg_P4+1]);
xis_bnd_P2 = reshape(X_Uman_bnd(:,1:n_state)',[n_state n_seg_P2+1]);
xis_bnd_P3 = reshape(X_Sman_bnd(:,1:n_state)',[n_state n_seg_P3+1]);
...
xis_bnd = cat(2,xis_bnd_revi,xis_bnd_P1(:,2:end),xis_bnd_P2(:,2:end),xis_bnd_P3(:,2:end),xis_bnd_P4(:,2:end),xis_bnd_revf(:,2:end));
...
sis_var_TF = cat(3,sis_var_revi,sis_var_P1,sis_var_P2,sis_var_P3,sis_var_P4,sis_var_revf);
