...
colt.Tmax_dim = 1.0; % [N] maximum s/c thrust
...
%-------------------------------------------------------------------------%
% Method 2 - Create Guess with Orbit "Spooling" and Trajectory from Collocation %
%-------------------------------------------------------------------------%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Perform Natural Parameter Continuation in Tmax %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% % Define initial and final Tmax
% Tmax_init = 1000/1000; % [N]
% Tmax_fin = 10/1000; % [N]
% 
% % Define initial and minimum step size for natural parameter continuation
% np_stp = 10/1000; % [N]
% np_stp_min = 0.001/1000; % [N]
% 
...
% colt_i = colt; 
% 
% % Initialize iteration counter and Cont_Stop variable
% ii = 0;
% ii_convg = 0;
% Cont_Stop = false;
% 
% % Begin continuation while loop
% while colt_i.Tmax_dim >= Tmax_fin && ~Cont_Stop
%     
%     % Iterate iteration counter
%     ii = ii + 1;
%     
%     % Print iteration number and step size
%     fprintf('\n Iteration %1.0f with Thrust Level %0.6f [mN] and Step Size %0.6f [mN] \n',ii,colt_i.Tmax_dim*1000,np_stp*1000)
%     
%     % Run direct transcription
%     [Z,x_bnd,t_var,t_bnd,C,colt] = DirectTrans(Z0_i,t_bnd_i,t_var_i,colt_i);
%     
%     % Store collocation results if convergence is obtained 
%     if ~colt.max_iter_chk % if collocation converges
%         
%         % Iterate convergence counter
%         ii_convg = ii_convg + 1;
%                
%         % Store results
%         Z_store{ii_convg} = Z;
%         x_bnd_store{ii_convg} = x_bnd;
%         t_bnd_store{ii_convg} = t_bnd;
%         t_var_store{ii_convg} = t_var;
%         C_store{ii_convg} = C;
%         colt_store{ii_convg} = colt;
% 
%         % Use result of current iteration as initial guess for the next iteration
% %         Z0_i = Z;
% %         t_bnd_i = t_bnd;
% %         t_var_i = t_var;
% %         colt_i = colt;
%         
%         % Note: When collocation converges the current step size is maintained
%         
%     elseif colt.max_iter_chk % if collocation does not converge
%         
%         % If min step size is already reached then no further step 
%         % reduction can be performed and Cont_Stop is set to true
%         if np_stp == np_stp_min
%             Cont_Stop = true;
%         end
%         
%         % Adjust step size
%         if np_stp/2 > np_stp_min 
%             % Reduce step size by half
%             np_stp = np_stp/2;
%         else
%             % Set step size to minimum value
%             np_stp = np_stp_min;
%         end
%         
%         % Reset thrust level to value before most recent step
%         colt_i.Tmax_dim = colt_store{ii_convg}.Tmax_dim;
%         colt_i.Tmax = colt_store{ii_convg}.Tmax;
% 
%         % Note: When collocation does not converge the initial guess is not updated
%         
%     end % end convergence check if statement
%     
%     % Step in dimensional Tmax
%     colt_i.Tmax_dim = colt_i.Tmax_dim - np_stp; % [N] maximum s/c thrust
%     colt_i.Tmax = colt_i.Tmax_dim*(colt.t_ch^2/(1000*colt.l_ch*colt.m_ch));
%    
% end % end natural parameter continuation loop

% Save or load collocation result
% save('DRO2L2NRHO_NoOpt_rev55_n100_TmaxCont_1k210_RevGuess','Z_store','x_bnd_store','t_var_store','t_bnd_store','C_store','colt_store')
load DRO2L2NRHO_NoOpt_rev55_n100_TmaxCont_1k210
...
% load Transfer_4to1NRHOtoDRO_dvpt05depart_OptwColl_Tmax1
...
% Run plotting script
run('Plot_Example_Direct_DRO2NRHO_EM_TmaxCont')
