function [X,T_half,DF_fin,sshoot] = SingShoot_Halo_PArc(X0,T0_half,sshoot)
% function [X,T_half,DF_fin,sshoot] = SingShoot_Halo_PArc(X0,T0_half,sshoot)
% 
...
% that are symmetric about the xz-plane, this includes the Halo and
% Vertical families of orbits.
%
% INPUTS:
...
% Extract necessary parameters for pseudo-arclength continuation
Z_prev = sshoot.Z_prev;
deltZ_prev = sshoot.deltZ_prev;
delt_s = sshoot.delt_s;
...
% Setup Integration
tspan = [0 Z(4)]; 
ic = [Z(1), X0(2), Z(2), X0(4), Z(3), X0(6), phi0];

% Integrate Trajectory
% [x,t,~,~,~] = GslInteg_STM_events(ic,tspan,mu,event_id,event_state,dir,stop);
...
F = [xf(2); xf(4); xf(6)]; % y, xdot, and zdot at final time must equal zero
...
DF_new = [phif(2,1), phif(2,3), phif(2,5), xfdot(2);...
          phif(4,1), phif(4,3), phif(4,5), xfdot(4);...
          phif(6,1), phif(6,3), phif(6,5), xfdot(6)];
